	...
	private class HandleType
	{
		...
		internal void Add(IntPtr handle)
		{
			if (!(handle == IntPtr.Zero))
			{
				...
				lock (internalSyncObject)
				{
					...
					{
						System.Internal.HandleCollector.HandleAdded(name, handle, currentHandleCount);
					}
				}
				if (flag && flag)
				{
		...
		internal IntPtr Remove(IntPtr handle)
		{
			...
			lock (internalSyncObject)
			{
				...
				System.Internal.HandleCollector.HandleRemoved(name, handle, currentHandleCount);
				return handle;
			}
		}
	}
